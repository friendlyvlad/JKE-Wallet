...
/**
 * A UI component built to modify Check entities. The used superclass
 ...
 * JPA implementation. Check out annotations in Check objects email field and
 * how they automatically reflect to the configuration of related fields in UI.
 * </p>
 */
public class CheckForm extends AbstractForm<Check> {
    ...
    DateField date = new DateField("Check Date");
    
    TypedSelect<String> account = new TypedSelect().withCaption("Account");
    ...
    protected Component createContent() {
        ...
        return new MVerticalLayout(
                new Header("Check Details").setHeaderLevel(3),
                new MFormLayout(
                        amount,
                        date,
                        account,
    ...
    void init() {
        ...
        account.setOptions(aservice.findAccounts(authname));
        account.setStyleName(ValoTheme.OPTIONGROUP_HORIZONTAL);
        
        setSavedHandler(new SavedHandler<Check>() {

            @Override
            public void onSave(Check entity) {
                try {
                    ...
                    /*
                     * The Check object uses optimitic locking with the 
                     * version field. Notify user the editing didn't succeed.
                     */
                    Notification.show("The Check was concurrently edited "
        ...
        setResetHandler(new ResetHandler<Check>() {

            @Override
            public void onReset(Check entity) {
                refrehsEvent.fire(entity);
            }
        });
        setDeleteHandler(new DeleteHandler<Check>() {
            @Override
            public void onDelete(Check entity) {
     ...
     */
    @Inject
    @CheckEvent(CheckEvent.Type.SAVE)
    javax.enterprise.event.Event<Check> saveEvent;

    @Inject
    @CheckEvent(CheckEvent.Type.REFRESH)
    javax.enterprise.event.Event<Check> refrehsEvent;

    @Inject
    @CheckEvent(CheckEvent.Type.DELETE)
    javax.enterprise.event.Event<Check> deleteEvent;
}
